...
import { DEFAULT_ADJUSTMENTS, SMART_EFFECTS, type ScanPage, type ScannerWorkerRequest } from './types'
...
describe('scanner request scheduling', () => {
  it('keeps only the newest queued preview', async () => {
    ...
    worker.respond({ id: worker.messages[0].id, type: 'rendered', blob: new Blob(['first']), width: 100, height: 80 })
    await first
    expect(worker.messages).toHaveLength(2)
    expect(worker.messages[1]).toMatchObject({ type: 'render', page: { adjustments: { brightness: 2 } } })

    worker.respond({ id: worker.messages[1].id, type: 'rendered', blob: new Blob(['third']), width: 100, height: 80 })
    await third
  })
  ...
  it('prewarms once and reuses an identical export render', async () => {
    ...
    expect(worker.messages).toHaveLength(2)
    worker.respond({ id: worker.messages[1].id, type: 'rendered', blob: new Blob(['export']), width: 3000, height: 2250 })
